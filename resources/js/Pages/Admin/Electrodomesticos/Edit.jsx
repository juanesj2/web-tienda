...
export default function Edit({ electrodomestico }) {
    const { t } = useLanguage();
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        eficiencia_energetica: electrodomestico.eficiencia_energetica || '',
        stock: electrodomestico.stock || '0',
    });

    const submit = (e) => {
        e.preventDefault();
        put(route('electrodomesticos.update', electrodomestico.id));
    };

    return (
        <MainLayout>
            <Head title={`Editar ${electrodomestico.nombre}`} />
            <div className={`min-vh-100 pt-5 pb-5 ${isDarkMode ? 'bg-dark' : 'bg-light'}`} style={{ transition: 'background-color 0.3s ease' }}>
                <div className="container mt-5 pt-4">
                    <div className="mb-4">
                        ...
                        <h2 className={`fw-bold mb-0 ${isDarkMode ? 'text-white' : 'text-dark'}`}>Editar electrodoméstico</h2>
                    </div>
                        ...
                        <div className="card-body p-4 p-md-5">
                            <form onSubmit={submit}>
                                <div className="row g-4">
                                    ...
                                    <div className="col-md-4">
                                        <label className={`form-label fw-semibold ${isDarkMode ? 'text-light' : ''}`}>Eficiencia</label>
